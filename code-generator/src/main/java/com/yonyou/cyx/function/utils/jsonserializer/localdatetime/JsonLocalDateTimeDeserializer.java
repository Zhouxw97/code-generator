    ...
    public LocalDateTime deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        try {
            if (StringUtils.isNullOrEmpty(date)) {
                ...
                return LocalDateTime.parse(date, DateTimeFormatter.ofPattern(FunctionConstants.FULL_DATE_TIME_FORMAT));
            }
        } catch (Exception e) {
//            throw new JsonSerializeException(e);
            LoggerFactory.getLogger(JsonLocalDateTimeDeserializer.class).error("json转LocalDateTime失败！默认为null");
            return null;
        }
    }
